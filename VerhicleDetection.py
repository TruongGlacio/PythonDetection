...
class VerhicleDetection:
    ...
    def InitialDetector(self, shape_detector_Path, detectorPath):
        ...
        self.detector = dlib.fhog_object_detector(detectorPath)# deserialize .svm file
    ...
    def VerhicleDetection(self, frame):
        ...
        scalar = (255,0,0)
        print("Number of verhicle detected: {}".format(len(dets)))
        #cv2.putText(frame, "Number of phone detected: {}".format(len(dets)), (p.x + 4, p.y), cv2.FONT_HERSHEY_SIMPLEX, 0.25, (255, 255, 255))
        if len(dets)> 0:    
            cv2.putText(frame,"Verhicle detected: {}".format(len(dets)), (20,140), cv2.FONT_HERSHEY_SIMPLEX,0.5, scalar)
        for d in dets:
